...
export default new Vuex.Store({
  ...
  actions: {
    async getStartingMode(context, date) {
      // 获取启动方式分组的数据
      context.commit("changeLoading"); // 加载动画
      let arg = arguments[1] || [GetDateStr(-7), GetDateStr(-1)];
      let url = `/accesslayer/NewsdailyPVUV/GetSimgleTrendChart?sdate=${
        arg[0]
      }&edate=${arg[1]}`;
    ...
    async getChannelsData(context) {
      ...
      let url = `/accesslayer/NewsdailyPVUV/GetChanelPvUvData?sdate=${
        arg.date[0]
      }&edate=${arg.date[1]}&channels=${arg.channel}`;
  ...
  mutations: {
    ...
    changeChannelsData(state, v) {
      ...
      let categories = res[CN[0]]["categories"].map(item => {
        // 时间,横坐标
        return `${item.slice(4, 6).match(/[1-9]+/)[0]}月${item.slice(6)}日`;
      });
