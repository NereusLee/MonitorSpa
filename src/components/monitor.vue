<template>
    <div class="monitor">
        ...
        <!--<el-row>-->
            <ul class="charts">
                <el-col :span="24" v-for="(x,index) in list" :key="x.id">
                    <li>
...
<script>
    ...
    export default {
        ...
        data(){
            return {
                kuaibao:{
                    ...
                    list:[
                        {attrid:463214,title:'快报 golang接入层 内搜推荐兜底数'},
                        {attrid:456035,title:'快报 golang接入层 请求总量'},
                        {attrid:455985,title:'快报 golang接入层 请求成功量'},
                        {attrid:475112,title:'快报 golang接入层 推荐条数少于请求条数'}
                    ],
                    menuList:['接入层']
                },
                news:{
                    ...
                    list:[
                        {attrid:435024,title:'新闻 接入层 列表页总调用量'},
                        {attrid:435027,title:'新闻 接入层 要闻频道调用量'},
                        {attrid:484451,title:'新闻 接入层 要闻推荐兜底调用量'},
                        {attrid:484452,title:'新闻 接入层 二级频道推荐兜底调用量'},
                        {attrid:435498,title:'新闻 push推送 地域推送数请求总量'}
                    ],
                    menuList:['接入层','push']
                },
                describe:'',
        ...
        methods:{
            ...
            getData(list){
                let ids = this.$route.params.type=='kuaibao'?this.kuaibao:this.news;
                this.getMonitorData({mixid:ids.mixid,list:list})
            },
